...
ot.TextOperation = (function () {
  ...
  TextOperation.prototype.retain = function (n) {
    ...
    this.baseLength += n;
    this.targetLength += n;
    // 操作合并：最后一个操作是retain的话则被当前retain替换掉
  ...
  TextOperation.prototype.insert = function (str) {
    ...
    if (isInsert(ops[ops.length-1])) {
      // Merge insert op.
      // 操作合并：最后一个操作是insert，将两个插入的字符拼在一起
  ...
  // Case：undo和redo的时候希望一次性撤回编辑的整个句子，或撤销一串删除操作（按住删除键）
  // How：通过将undo stack上的操作进行合并
  // 什么样的操作需要合并：
  // 1. 简单操作不需要合并
  // 2. 
  ...
  TextOperation.transform = function (operation1, operation2) {
    ...
    while (true) {
      ...
      if (isRetain(op1) && isRetain(op2)) {
        ...
        // 1. 更新minLength
        // 2. retain偏小的一个获取下一个op
        // 3. retain偏大的一个bigger = bigger - smaller
        // 4. newOp都put一个retain(minLength)的操作
        if (op1 > op2) {
          minl = op2;
          op1 = op1 - op2;
          op2 = ops2[i2++];
      ...
      // next two cases: delete/retain and retain/delete
        ...
        if (-op1 > op2) {
          // case：delete大，retain小
          // 那就别retain了，delete拆分成两个操作
          minl = op2;
          op1 = op1 + op2;
          op2 = ops2[i2++];
        } else if (-op1 === op2) {
