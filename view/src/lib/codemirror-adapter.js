...
ot.CodeMirrorAdapter = (function (global) {
  ...
  /**
   * CodeMirror编辑器Adapter
   * 负责将CodeMirror的Event对象转换成TextOperation
   * @param {CodeMirror} cm CodeMirror编辑器实例
   */
  function CodeMirrorAdapter (cm) {
    ...
    /**
     * CodeMirror的事件类型
     * CodeMirror事件文档：https://codemirror.net/doc/manual.html#events
     */
    ...
    // where the change started and ended (for example, it might be 
    // {ch:0, line:18} if the position is at the beginning of line #19). 
    // text is an array of strings representing the text that replaced 
    // the changed range (split by line). removed is the text that used 
    // to be between from and to, which is overwritten by this change. 
    ...
    // 将Adapter内的方法绑定到CodeMirror里，作为它的事件回调
  ...
  // CodeMirror事件对象转换成TextOperation对象的核心方法
  ...
  CodeMirrorAdapter.operationFromCodeMirrorChanges = function (changes, doc) {
    ...
    for (var i = changes.length - 1; i >= 0; i--) {
      ...
      // 不考虑事件类型，把事件全用上，最后用compose进行事件合并
  ...
  // 编辑器内的改动会同时触发onChange和onCursorActivity事件
  // 因此当onChange被触发的时候，就修改标识位，让onCursorActivity能够辨别
  ...
  // 应用修改的时候会触发Event，让这次apply事件不被处理
